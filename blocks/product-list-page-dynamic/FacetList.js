  ...
  render() {
    ...
    return html`<div class="price-facet">
        <div class="price-slider">
          <input type="range" ref=${this.minRef} id="price-slider-min" defaultValue=${selectionMin} min=${min} max=${max} step="1" onchange=${this.onChange} oninput=${() => this.onChange(false)} autocomplete="off" />
          <input type="range" ref=${this.maxRef} id="price-slider-max" defaultValue=${selectionMax} min=${min} max=${max} step="1" onchange=${this.onChange} oninput=${() => this.onChange(false)} autocomplete="off" />
        </div>
...
function Facet({
  ...
  const renderOptions = () => {
    ...
    if (displayType === 'checkbox') {
      return buckets.map((bucket) => {
        const data = (attribute === 'categories') ? bucket.title : bucket.id;
        return html`<li>
          <input type="checkbox" name="facet-${attribute}" id="facet-${data}" value=${data} checked=${selection.includes(data)} onClick=${handleClickMultiple} />
          <label for="facet-${data}">
            ${bucket?.name || bucket.title} <span class="count">${bucket.count}</span>
          </label>
        </li>`;
      });
    }
    if (displayType === 'radio') {
      return buckets.map((bucket) => {
        const data = (attribute === 'categories') ? bucket.title : bucket.id;
        return html`<li>
        <input type="radio" name="facet-${attribute}" id="facet-${data}" value=${data} checked=${selection.includes(data)} onClick=${handleClickSingle} />
        <label for="facet-${data}">
  ...
  return html`<div class="facet ${displayType} ${displayStyle || ''}">
    ...
    <div class="facet-content">
        ${attribute !== 'price' ? html`<ol>${renderOptions()}</ol>` : renderOptions()}
    </div>
  </div>`;
}
